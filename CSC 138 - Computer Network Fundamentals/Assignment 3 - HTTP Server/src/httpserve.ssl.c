...
void _RedirectToIndex(SSL *ssl);
void _BadRequestException(SSL *ssl);
void _ForbiddenException(SSL *ssl);
void _NotFoundException(SSL *ssl);
void _MethodNotAllowedException(SSL *ssl);
void _UnsupportedMediaTypeException(SSL *ssl);
void _InternalServerErrorException(SSL *ssl);
...
void handle_ssl_request(SSL *ssl)
{
    ...
    _MethodNotAllowedException(ssl);
    loggerf(INFO, "NOT ALLOWED Method: %s | Payload: %s", method, request);
}
